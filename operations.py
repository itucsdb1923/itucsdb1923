    ...
    with dbapi2.connect(url) as connection:
        with connection.cursor() as cursor:
            ...
            for item_id, title, description, year, imdb_score, score, votes, director_id in a:
                x = {"item_id" : item_id, "title" : title, "description" : description, "year" : year, "imdb_score" : imdb_score, "score" : score, "votes" : votes}
    ...
    with dbapi2.connect(url) as connection:
        with connection.cursor() as cursor:
            ...
            for item_id, title, description, year, page_num, score, votes, author_id in a:
                x = {"item_id" : item_id, "title" : title, "description" : description, "year" : year, "page_num" : page_num, "score" : score, "votes" : votes}
                statement = """SELECT NAME FROM PERSON WHERE (ID = %s)"""
    ...
    with dbapi2.connect(url) as connection:
        with connection.cursor() as cursor:
            ...
            for item_id, title, album, year, score, votes, singer_id in a:
                x = {"item_id" : item_id, "title" : title, "album" : album, "year" : year, "score" : score, "votes" : votes}
                statement = """SELECT NAME FROM PERSON WHERE (ID = %s)"""
...
def getMovie(movie_id): 
    with dbapi2.connect(url) as connection:
        with connection.cursor() as cursor:
            ...
            a = cursor.fetchall()
            (item_id, title, description, year, imdb_score, score, votes, director_id) = a[0]
            x = {"item_id" : item_id, "title" : title, "description" : description, "year" : year, "imdb_score" : imdb_score, "score" : score, "votes" : votes}
            statement = """SELECT NAME FROM PERSON WHERE (ID = %s)"""
            cursor.execute(statement, (director_id,))
            x["director"] = cursor.fetchone()[0]
...
def getBook(book_id):  
    with dbapi2.connect(url) as connection:
        with connection.cursor() as cursor:
            ...
            a = cursor.fetchall()
            (item_id, title, description, year, page_num, score, votes, author_id) = a[0]
            x = {"item_id" : item_id, "title" : title, "description" : description, "year" : year, "page_num" : page_num, "score" : score, "votes" : votes}
            statement = """SELECT NAME FROM PERSON WHERE (ID = %s)"""
            cursor.execute(statement, (author_id,))
            x["author"] = cursor.fetchone()[0]
